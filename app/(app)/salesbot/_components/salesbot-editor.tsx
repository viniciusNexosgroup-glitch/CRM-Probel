...
import type { SalesbotEdge, SalesbotFlow, SalesbotNode, SalesbotNodeType } from "@/lib/salesbot/types";
import { saveSalesbotGraphAction, updateSalesbotFlowStatusAction } from "../actions";

type Option = { id: string; name?: string | null; full_name?: string | null; email?: string | null; title?: string | null; color?: string | null; file_type?: string };
...
export function SalesbotEditor({
  ...
  const [nodes, setNodes] = useState<SalesbotNode[]>(() =>
    initialNodes.length > 0 ? initialNodes : [createNode("send_message", 120, 120)]
  );
    ...
    <div className="flex-1 min-h-0 grid grid-cols-[260px_1fr_320px] bg-wa-bg">
      ...
      <aside className="bg-wa-panel border-l border-wa-border flex flex-col min-h-0">
        <div className="h-11 px-4 border-b border-wa-border flex items-center justify-between">
          ...
          {selectedNode && (
            <button type="button" onClick={removeSelected} className="text-wa-textSecondary hover:text-red-400">
              <X className="h-4 w-4" />
            </button>
          )}
        </div>
